...
class JobDatabase:
    ...
    def drop_all_tables(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute('DROP TABLE IF EXISTS scraping_runs')
    ...
    def insert_job(self, job_data):
        ...
        with self.get_connection() as conn:
            ...
            # Check if job exists
            cursor.execute('SELECT id FROM jobs WHERE external_id = ?', (job_data['external_id'],))
            ...
            if existing:
                ...
                cursor.execute('''
                    UPDATE jobs SET
                        ...
                        state = ?, country = ?, employment_type = ?, department = ?,
                        apply_url = ?, posted_date = ?, job_function = ?,
                        experience_level = ?, salary_range = ?, remote_type = ?,
                        status = ?, updated_at = ?
                    WHERE external_id = ?
                ''', (
