...
logger = setup_logger('ingersollrand_scraper')
...
class IngersollRandScraper:
    ...
    def parse_location(self, location_str):
        ...
        SAP SuccessFactors format: 'City, STATE_CODE, COUNTRY_CODE, ZIP'
        e.g. 'Bangalore, KA, IN, 560 029'
        """
        ...
        # For 4-part format: City, State, Country, ZIP
        if len(parts) >= 3:
            country_part = parts[2].strip()
            if country_part == 'IN':
                result['country'] = 'India'
            elif country_part in ['AE', 'SA', 'QA', 'KW', 'BH', 'OM']:
                result['country'] = country_part
            else:
                result['country'] = country_part
        if 'India' in location_str or ', IN,' in location_str:
            result['country'] = 'India'
        return result
    ...
    def _is_india_job(self, location):
        ...
        SAP SuccessFactors uses location format: 'City, STATE, COUNTRY_CODE, ZIP'
        e.g. 'Surendranagar, GJ, IN, 363035' where IN = India country code.
        """
        india_keywords = [
            ...
            'sahibabad', 'naroda', 'hosur', 'mohali', 'bahadurgarh',
            'surendranagar'
        ]
        location_lower = location.lower()

        # Check city/country name keywords
        if any(kw in location_lower for kw in india_keywords):
            return True

        # Check for India country code 'IN' in comma-separated location parts
        # Format: "City, State, IN, ZIP"
    ...
    def scrape(self, max_pages=MAX_PAGES_TO_SCRAPE):
        """Scrape jobs from Ingersoll Rand SAP SuccessFactors career site."""
        all_jobs = []
        seen_ids = set()
            ...
            for page in range(max_pages):
                ...
                for row in job_rows:
                    try:
                        ...
                        # Filter for India jobs (since URL covers Middle East, India and Africa)
