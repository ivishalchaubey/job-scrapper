...
logger = setup_logger('snowflake_scraper')
        ...
        try:
            ...
            for job in jobs:
                try:
                    ...
                    # Snowflake uses "IN-" prefix for India locations (e.g., "IN-Pune", "IN-Bangalore-MSO")
                    is_india = (
                        any(kw in location.lower() for kw in india_keywords)
                        or country.lower() == 'india'
                        or bool(re.match(r'^IN-', location))
                    )
                    if not is_india:
                        continue
                    ...
                    # Parse city from "IN-Pune" or "IN-Bangalore-MSO" format
                    if not city and location.startswith('IN-'):
                        city_part = location[3:]  # Remove "IN-"
                        city = city_part.split('-')[0]  # Remove "-MSO" suffix
