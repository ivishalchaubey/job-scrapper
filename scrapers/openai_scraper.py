...
logger = setup_logger('openai_scraper')
        ...
        try:
            ...
            for job in jobs:
                try:
                    ...
                    # Check secondaryLocations too
                    secondary_india = False
                    for sec in job.get('secondaryLocations', []):
                        sec_addr = sec.get('address', {}) or {}
                        sec_postal = sec_addr.get('postalAddress', {}) or {}
                        sec_loc = sec.get('location', '')
                        if sec_postal.get('addressCountry', '').lower() == 'india' or any(kw in sec_loc.lower() for kw in india_keywords):
                            secondary_india = True
                            break
