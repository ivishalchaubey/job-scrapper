...
logger = setup_logger('confluent_scraper')


class ConfluentScraper:
    def __init__(self):
        self.company_name = 'Confluent'
        self.api_url = 'https://api.ashbyhq.com/posting-api/job-board/confluent'
        ...
        try:
            ...
            for job in jobs:
                try:
                    ...
                    # Confluent uses "IN Office Bangalore", "IN Remote India" patterns
                    is_india = (
                        any(kw in location.lower() for kw in india_keywords)
                        or country.lower() == 'india'
                        or bool(re.match(r'^IN\s', location))
                    )
                    if not is_india:
                        continue
                    ...
                    # Parse city from "IN Office Bangalore" format
                    if not city and location.startswith('IN '):
                    ...
                    if job.get('isRemote'):
                        remote_type = 'Remote'
                    # Also check location for remote hint
